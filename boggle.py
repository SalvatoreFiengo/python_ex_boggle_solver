...
def all_grid_neighbours(grid):
        """
        Get all possible neighbours for each pos in the grid
        """ 
        neighbours = {}
...
def path_to_word(grid, path):
        """
        all letters of a path to a string
        """
        return ''.join([grid[p] for p in path])

def search(grid, dictionary):
        """
        search trough the paths to locate words by matching strings
...
def main():
    """
    this functionality will run the whole program
    """
    grid = make_grid(3,3)
